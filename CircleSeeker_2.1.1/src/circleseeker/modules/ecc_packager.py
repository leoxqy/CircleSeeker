...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("-s", "--sample-name", required=True, dest="sample_name",
    ...
    p.add_argument("--inferred-dir", required=True, help="Directory containing inferred eccDNA files.")
...
def run(args) -> int:
    ...
    for d in [base, U_dir, M_dir, C_dir, I_dir]:
        ensure_dir(d, dry=args.dry_run, verbose=args.verbose or True)

    # Copy Confirmed U
    copy_file(first_glob(uecc_dir, "*_UeccDNA_C.fasta"), U_dir / f"{sample}_UeccDNA_C.fasta",
              overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)
    copy_file(first_glob(uecc_dir, "*_UeccDNA.bed"), U_dir / f"{sample}_UeccDNA.bed",
              overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)
    copy_file(first_glob(uecc_dir, "*_UeccDNA.core.csv"), U_dir / f"{sample}_UeccDNA.core.csv",
              overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)

    # Copy Confirmed M
    copy_file(first_glob(mecc_dir, "*_MeccDNA_C.fasta"), M_dir / f"{sample}_MeccDNA_C.fasta",
              overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)
    copy_file(first_glob(mecc_dir, "*_MeccSites.bed"), M_dir / f"{sample}_MeccSites.bed",
              overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)
    copy_file(first_glob(mecc_dir, "*_MeccBestSite.bed"), M_dir / f"{sample}_MeccBestSite.bed",
              overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)
    copy_file(first_glob(mecc_dir, "*_MeccSites.core.csv"), M_dir / f"{sample}_MeccSites.core.csv",
              overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)

    # Copy Confirmed C
    copy_file(first_glob(cecc_dir, "*_CeccDNA_C.fasta"), C_dir / f"{sample}_CeccDNA_C.fasta",
              overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)
    copy_file(first_glob(cecc_dir, "*_CeccSegments.bed"), C_dir / f"{sample}_CeccSegments.bed",
              overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)
    copy_file(first_glob(cecc_dir, "*_CeccJunctions.bedpe"), C_dir / f"{sample}_CeccJunctions.bedpe",
              overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)
    copy_file(first_glob(cecc_dir, "*_CeccSegments.core.csv"), C_dir / f"{sample}_CeccSegments.core.csv",
              overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)

    # Copy Inferred
    copy_file(first_glob(inferred_dir, "*_simple.csv"), I_dir / f"{sample}_UeccDNA_I.csv",
              overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)
    copy_file(first_glob(inferred_dir, "*_UeccDNA_I.fasta"), I_dir / f"{sample}_UeccDNA_I.fasta",
              overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)
    copy_file(first_glob(inferred_dir, "*_chimeric.csv"), I_dir / f"{sample}_chimeric.csv",
              overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)
    copy_file(first_glob(inferred_dir, "*_CeccDNA_I.fasta"), I_dir / f"{sample}_CeccDNA_I.fasta",
              overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)

    # Copy required single-file inputs
    copy_file(merged_csv, dest_merged, overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)
    copy_file(html_in, dest_html, overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)
    copy_file(txt_in, dest_txt, overwrite=args.overwrite, dry=args.dry_run, verbose=args.verbose or True)
