...
def curate_ecc_tables(input_tsv: Path | str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for _, row in df_dedup.iterrows():
        ...
        if int(row["segment_count"]) == 1:
            ...
            regions = str(row["regions"]).strip().split(",")
            total_segments = len(regions)
            is_valid = all(":" in r and "-" in r for r in regions)
            if not is_valid:
                continue
            for seg_idx, region in enumerate(regions, 1):
