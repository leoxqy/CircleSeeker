"""
Minimap2 wrapper module for CircleSeeker2
...
class Minimap2(ExternalTool):
    ...
    def align(
        ...
        should_build_index = build_index_first if build_index_first is not None else self.config.build_index
    ...
    def _run_alignment_safe(self, reference: Path, reads: Path, output_bam: Path, temp_dir: Path) -> None:
        """
        ...
        self.logger.info(f"Running minimap2 alignment")
        ...
        self.logger.info(f"Sorting alignment to BAM")
    ...
    def _index_bam(self, bam_file: Path) -> None:
        ...
        self.logger.info(f"Creating BAM index")
        self.logger.debug(f"Command: {' '.join(command)}")
        ...
        try:
            ...
            self.logger.info(f"BAM index created successfully")
