...
class EccSummary:
    ...
    def collect_read_statistics(self, fasta_path: Path, processed_csv: Path) -> Dict:
        ...
        for subtype, count in ctcr_subtypes.items():
            ...
            self.ctcr_stats[total_pct_key] = round(count / total_reads * 100, 1) if total_reads > 0 else 0
    ...
    def _get_empty_eccdna_stats(self) -> Dict:
        ...
        for prefix in ['all', 'uecc', 'mecc', 'cecc']:
            ...
            for suffix in ['min_length', 'max_length', 'mean_length', 'median_length', 'mode_length', 'std_length']:
    ...
    def collect_overlap_statistics(self, overlap_json: Path) -> Dict:
        ...
        try:
            ...
            self.logger.info(f"Loaded overlap statistics successfully")
    ...
    def generate_text_summary(self, output_path: Optional[Path] = None) -> str:
        ...
        lines.append(f"CtcR Reads: {self.read_stats['ctcr_reads']:,} ({self.read_stats['ctcr_percentage']}%)")
        lines.append(f"  - CtcR-perfect: {self.ctcr_stats['ctcr_perfect_count']:,} ({self.ctcr_stats['ctcr_perfect_pct_ctcr']}% of CtcR)")
        lines.append(f"  - CtcR-hybrid: {self.ctcr_stats['ctcr_hybrid_count']:,} ({self.ctcr_stats['ctcr_hybrid_pct_ctcr']}% of CtcR)")
        lines.append(f"  - CtcR-inversion: {self.ctcr_stats['ctcr_inversion_count']:,} ({self.ctcr_stats['ctcr_inversion_pct_ctcr']}% of CtcR)")
        lines.append(f"Other Reads: {self.read_stats['other_reads']:,} ({self.read_stats['other_percentage']}%)")
        lines.append("")
        ...
        for prefix, name in [('all', 'All'), ('uecc', 'UeccDNA*'), ('mecc', 'MeccDNA'), ('cecc', 'CeccDNA*')]:
        ...
        lines.append("-" * 21)
        lines.append(f"Inferred UeccDNA: {self.overlap_stats['inferred_uecc_count']} sequences, "
                    f"{self.overlap_stats['uecc_overlap_count']} ({self.overlap_stats['uecc_overlap_pct']}%) overlap")
        lines.append(f"Inferred CeccDNA: {self.overlap_stats['inferred_cecc_count']} sequences, "
                    f"{self.overlap_stats['cecc_overlap_count']} ({self.overlap_stats['cecc_overlap_pct']}%) overlap")
        lines.append("")
...
<body>
    ...
    def process_all(self,
                   ...
                   processed_csv: Path,
                   merged_csv: Path,
                   overlap_json: Path) -> Tuple[str, str]:
...
def main():
    ...
    parser.add_argument(
        "-f", "--fasta",
        required=True,
        help="Path to original reads FASTA file"
    )
    
    parser.add_argument(
        "-p", "--processed",
        required=True,
        help="Path to processed reads CSV file"
    )
    
    parser.add_argument(
        "-m", "--merged",
        required=True,
        help="Path to merged eccDNA CSV file"
    )
    
    parser.add_argument(
        "-j", "--json",
        required=True,
        help="Path to overlap statistics JSON file"
    )
    ...
    parser.add_argument(
        "--text-only",
        action="store_true",
        help="Generate only text summary"
    )
