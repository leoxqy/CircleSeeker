...
class CircularDNAResult:
    ...
    def from_dataframe_row(cls, row: pd.Series) -> "CircularDNAResult":
        return cls(
            ...
            read_support=(
                int(row.get("read_support")) if row.get("read_support") is not None else None
            ),
        )
            ...
            if p and p.exists():
                try:
                    ...
                    try:
                        ...
                        self.logger.debug(f"Could not remove index file {idx_csi}: {exc}")
                    try:
                        ...
                        self.logger.debug(f"Could not remove index file {idx_tbi}: {exc}")
...
def _parse_args() -> Any:
    ...
    parser = argparse.ArgumentParser(description="Cyrcular-Calling - HiFi circular DNA detection")
    parser.add_argument("-b", "--bam", required=True, help="Input BAM file")
    parser.add_argument("-r", "--reference", required=True, help="Reference FASTA file")
    parser.add_argument("-o", "--output-dir", required=True, help="Output directory")
    parser.add_argument(
        "-s", "--sample-name", default="sample", help="Sample name (default: sample)"
    )
    parser.add_argument("--min-read-depth", type=int, default=2, help="Min read depth (default: 2)")
    parser.add_argument(
        "--min-split-reads", type=int, default=2, help="Min split reads (default: 2)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max-deletion-length", type=int, default=1000, help="Max deletion length (default: 1000)"
    )
    parser.add_argument("--fdr", type=float, default=0.2, help="FDR threshold (default: 0.2)")
    parser.add_argument(
        "--max-depth", type=int, default=200, help="Max sequencing depth (default: 200)"
    )
    parser.add_argument("--threads", type=int, default=4, help="Threads (default: 4)")
    parser.add_argument("--memory-limit", default="6G", help="Memory limit (default: 6G)")
