...
class UMeccClassifier:
    ...
    def read_blast_results(self, blast_file: Path) -> pd.DataFrame:
        ...
        try:
            ...
            df[ColumnStandard.LENGTH] = pd.to_numeric(split_cols[2], errors='coerce').fillna(0).astype(int)
            df[ColumnStandard.COPY_NUMBER] = pd.to_numeric(split_cols[3], errors='coerce').fillna(0).astype(float)
            
            # Validate length to avoid division by zero later
            invalid_length = df[ColumnStandard.LENGTH] <= 0
            if invalid_length.any():
                self.logger.warning(f"Found {invalid_length.sum()} entries with invalid length (<=0)")
        ...
        df.loc[valid_mask, 'Gap_Percentage'] = (
            (df.loc[valid_mask, 'gap_Length'].abs() / df.loc[valid_mask, ColumnStandard.LENGTH]) * 100
        ).round(2)
    ...
    def classify_blast_results(self, blast_df: pd.DataFrame, output_prefix: Path) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """
        ...
        self.logger.info(f"Processed {len(df):,} alignments from {df['query_id'].nunique():,} queries")
    ...
    def _save_classification_files(self, uecc_df: pd.DataFrame, mecc_df: pd.DataFrame,
                                 unclassified_df: pd.DataFrame, output_prefix: Path) -> None:
...
def _parse_args():
    ...
    parser.add_argument(
        "-i", "--input",
        required=True,
        help="Input BLAST result TSV file"
    )
    parser.add_argument(
        "-o", "--output",
        required=True,
