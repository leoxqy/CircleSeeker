...
class TandemToRing:
    ...
    def read_and_preprocess_data(self) -> pd.DataFrame:
        ...
        self.logger.info(f"Loaded {len(df)} regions from {df[ColumnStandard.READS].nunique()} reads")
    ...
    def separate_simple_complex(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        self.logger.info(f"Simple reads: {len(df_simple)}, Complex reads: {df_complex[ColumnStandard.READS].nunique()}")
    ...
    def process_complex_group_with_graph_optimized(self, group_df: pd.DataFrame) -> pd.DataFrame:
                ...
                for interval in overlaps:
                    ...
                    if other_idx != idx:
                        ...
                        if pair not in processed_pairs:
                            ...
                            overlap = min(row[ColumnStandard.END0], other_row[ColumnStandard.END0]) - max(row[ColumnStandard.START0], other_row[ColumnStandard.START0]) + 1
            ...
            for i in group_df.index:
                for j in group_df.index:
                    if i < j:
                        ...
                        overlap = min(group_df.loc[i, ColumnStandard.END0], group_df.loc[j, ColumnStandard.END0]) - \
                                 max(group_df.loc[i, ColumnStandard.START0], group_df.loc[j, ColumnStandard.START0]) + 1
        ...
        for component in nx.connected_components(G):
            if len(component) == 1:
                ...
                component_df['coverage_length'] = component_df[ColumnStandard.END0] - component_df[ColumnStandard.START0] + 1
...
def _parse_args():
    ...
    parser.add_argument(
        "-i", "--input",
        required=True,
        help="Input TideHunter result TSV file"
    )
    parser.add_argument(
        "-o", "--output",
        required=True,
        help="Output CSV file for classification"
    )
    parser.add_argument(
        "-c", "--circular-fasta",
        required=False,
