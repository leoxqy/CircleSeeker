...
class Pipeline:
    ...
    def show_steps(self, detailed: bool = False) -> None:
        ...
        for i, step in enumerate(self.STEPS, 1):
            ...
            click.echo(f"{status} Step {i:{idx_width}d}: {step_label:<{name_width}} - {step.description}")
    ...
    def _step_read_filter(self) -> None:
        """Step 10: Filter sequences using read_filter module (based on TandemToRing classification)."""
        ...
        self._set_result("read_filter_ctcr_reads", stats.csv_ctcr_reads)
    ...
    def _run_cresil_inference(self) -> None:
        ...
        if overview_tsv.exists():
            self.state.results[ResultKeys.CYRCULAR_OVERVIEW] = self._serialize_path_for_state(overview_tsv)
            # Count results
            import pandas as pd
    ...
    def _step_curate_inferred_ecc(self) -> list[str] | None:
        ...
        if not overview_path_obj:
            self.logger.warning("Cyrcular overview table missing; skipping inferred eccDNA curation")
            # Create empty results to allow pipeline to continue
            self.state.results[ResultKeys.INFERRED_SIMPLE_CSV] = None
            self.state.results[ResultKeys.INFERRED_CHIMERIC_CSV] = None
