...
> 注意：当前版本 `run_alignment` 步骤仅支持 minimap2；LAST 仅用于 `cecc_build` 步骤的复杂 eccDNA 检测。

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `aligner` | string | `minimap2` | 当前仅支持 `minimap2` |
| `min_identity` | float | `99.0` | 最小 identity 阈值（百分比） |
| `min_alignment_length` | int | `50` | 最小比对长度（bp） |
| `db_prefix` | Path | 无 | LAST 预建库前缀（仅用于 cecc_build） |
