...
14. **ecc_unify** – Merge confirmed and inferred results, generating unified CSV and overlap metrics.  
...
- **External dependencies** Ensure `tidehunter`, `blastn`, `cd-hit-est`, `minimap2`, `samtools`, `cresil` (or `cyrcular`) are available in `PATH`; they are validated via the shared `ExternalTool` base class.
