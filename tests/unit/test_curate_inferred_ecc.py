...
class TestWriteCuratedTables:
    def test_write_both_tables(self, tmp_path):
        ...
        assert simple_out.name == "test_output_simple.tsv"
        assert chimeric_out.name == "test_output_chimeric.tsv"
        
        # Check content
        simple_loaded = pd.read_csv(simple_out, sep='\t')
        assert len(simple_loaded) == 2
        assert list(simple_loaded['eccDNA_id']) == ['IUeccDNA1', 'IUeccDNA2']
        
        chimeric_loaded = pd.read_csv(chimeric_out, sep='\t')
...
class TestProcessEccDNA:
    ...
    def test_backward_compatibility(self, sample_cyrcular_output, tmp_path):
        ...
        simple_file = inferred_dir / "compat_test_simple.tsv"
        chimeric_file = inferred_dir / "compat_test_chimeric.tsv"
        ...
        # Verify content
        simple_df = pd.read_csv(simple_file, sep='\t')
...
class TestRealData:
    ...
    def test_process_eccDNA_with_fasta(self, lb_2_3_data, reference_fasta, tmp_path):
        ...
        # Check TSV files
        simple_tsv = inferred_dir / "full_test_simple.tsv"
        chimeric_tsv = inferred_dir / "full_test_chimeric.tsv"
...
class TestIntegration:
    def test_full_pipeline(self, tmp_path):
        ...
        assert (tmp_path / "integration_test_simple.tsv").exists()
        assert (tmp_path / "integration_test_chimeric.tsv").exists()
