...
import pysam
import pandas as pd  # <-- Newly added: Use pandas to read and write tables
...
class Astrologer:
    ...
    def __init__(
        ...
        no_reads_list=False,
        min_score=10.0,
        output_filter="output_filter.csv",  # Final output as CSV
        # Parameters related to generate_local_consensus
        input_txt="output_filter.csv",
        ref_fa="ref.fa",
        reads_fa="reads.fa",
        output_consensus="my_output_with_eSeq.csv",  # Default output as CSV
        threads=4,
        minimap2_threads=4,
    ):
        """
        ...
        # Constants used in robust_highcov_filter
    ...
    def median_absolute_deviation(self, data, scale_factor=1.4826):
        """
        Computes the median absolute deviation (MAD) for a list of values.
        Returns (median, MAD * scale_factor).
        """
    ...
    def load_coverage(self, bed_file):
        """
        Reads the .bed file line by line and stores the coverage data into:
            self.regions = [(chrom, start, end, cov), ...]
            self.coverages = [cov1, cov2, ...]
        """
        regions = []
        coverages = []
        with open(bed_file, 'r') as f:
            for line in f:
                ...
                cov = float(cov_str)
                if end > start:  # ensure valid region
    ...
    def identify_high_coverage_regions_robust(self):
        """
        1) First "protect" regions with coverage > 10 => candidate_flags = True
        2) For regions with coverage <= 10, use MAD for robust testing
        3) If MAD=0 (extreme case), coverage > median_cov * 2 is considered high coverage
        4) Consider self.min_length requirement
        """
        # Set extreme high coverage threshold to 10
        extreme_cov_threshold = 10

        # Collect all regions with coverage <= 10 for MAD testing
        normal_cov_values = []
        normal_indices = []
        for i, (chrom, start, end, cov) in enumerate(self.regions):
            if cov <= extreme_cov_threshold:
                normal_cov_values.append(cov)
                normal_indices.append(i)

        # Calculate median and MAD for normal regions
        if len(normal_cov_values) > 0:
            median_cov, mad_cov = self.median_absolute_deviation(normal_cov_values)
        else:
            # If no regions with coverage <= 10, set to 0 and skip MAD logic
            median_cov, mad_cov = 0, 0

        # Initialize flags
        candidate_flags = [False] * len(self.regions)

        # First, mark regions with coverage > 10 as True (high_cov)
        for i, (chrom, start, end, cov) in enumerate(self.regions):
            length = end - start + 1
            if cov > extreme_cov_threshold and length >= self.min_length:
                candidate_flags[i] = True

        # Perform MAD testing for regions with coverage <= 10
        for i in normal_indices:
            chrom, start, end, cov = self.regions[i]
            length = end - start + 1
            if length < self.min_length:
                # Length too short, do not mark
                candidate_flags[i] = False
                continue

            # If MAD = 0, use simple multiplier for testing
            if mad_cov == 0:
                if cov > median_cov * 2 and length >= self.min_length:
                    candidate_flags[i] = True
            else:
    ...
    def filter_zero_cov_low(self, regions, candidate_flags):
        """
        Original script function: If a region is not high (False), but not equal to 0 (cov>0), also keep it for later processing.
        """
        filtered = []
        new_flags = []
    ...
    def merge_by_gap(self, regions, candidate_flags):
        """
        Merge adjacent regions according to gap_threshold
        """
        ...
        for (r, f) in combined:
            ...
            if current_cluster is None:
                ...
                # If gap between current cluster and this region is less than threshold, merge
    ...
    def fetch_reads_in_region(self, chrom, start, end):
        """
        Return all qualified reads and their statistics in this region
        """
        ...
        for read in samfile.fetch(chrom, start, end):
            ...
            # Check for split reads (cigar or SA tag)
    ...
    def fetch_reads_for_merged(self, merged):
        """
        Fetch reads set and related statistics for each region after merge_by_gap.
        """
        for m in merged:
            (
    ...
    def merge_by_reads(self, merged):
        """
        If adjacent regions have a large overlap in reads, merge them again.
        """
        changed = True
        while changed:
            changed = False
    ...
    def compute_average_coverage(self, final_merged):
        """
        Calculate weighted average coverage (weighted_cov) for merged regions.
        """
    ...
    def calculate_score_and_write_output(self, merged_list, out_file):
        """
        Calculate scores and output. Here using pandas.DataFrame to write CSV.
        """
        scale_factor = 100.0 / 120.0

        # Construct output table columns
        header = [
        ...
        for m in merged_list:
            ...
            if num_reads > 0:
                ratio = float(weighted_cov) / num_reads if weighted_cov else 0.0
            ...
            row_data = {
                ...
                "length": length,
                "num_reads": num_reads,
                "high_cov": 1 if m['high_cov'] else 0,
                "weighted_cov": f"{float(weighted_cov):.2f}" if weighted_cov else "0.00",
                "split_read_count": split_count,
                "split_read_ratio": f"{split_ratio:.3f}",
                "Mean_MAPQ": f"{Mean_MAPQ:.2f}",
                "High_MAPQ_ratio": f"{High_MAPQ_ratio:.3f}",
                "score": f"{final_score:.2f}",
                "Avg_Read_Length": f"{avg_read_len:.2f}",
                "Avg_Split_Read_Length": f"{avg_split_read_len:.2f}"
            }
            if not self.no_reads_list:
                row_data["reads_list"] = ",".join(m['reads'])

            rows.append(row_data)

        df_out = pd.DataFrame(rows, columns=header)
        # Write to CSV (default comma separator, index=False)
        df_out.to_csv(out_file, index=False)

    def run_robust_highcov_filter(self):
        """
        Read coverage -> identify_high_coverage_regions_robust() -> filter -> merge -> calculate scores and output (CSV).
        """
    ...
    def process_region(self, r, reads_dict, ref, temp_dir):
        """
        For a given region, extract local reference sequence + local reads, align and generate local consensus sequence.
        """
        chrom = r["chrom"]
        start = int(r["start"])
        end = int(r["end"])
        ...
        # Modify reference sequence header
    ...
    def run_generate_local_consensus(self):
        """
        Read region information from self.input_txt, extract local sequence and generate local consensus for each region.
        Here using pandas to read CSV (default comma separator), and output CSV as well.
        """
        ...
        # ========= Use pandas to read CSV (default comma separator) =========
        try:
            df = pd.read_csv(self.input_txt, dtype=str)
        except Exception as e:
            self.logger.error(f"Failed to read input_txt with pandas: {e}")
            sys.exit(1)

        # Check required columns
        required_cols = {"chrom", "start", "end"}
        if not required_cols.issubset(df.columns):
            self.logger.error("Input CSV missing columns: chrom, start, end.")
            sys.exit(1)

        if "reads_list" not in df.columns:
            self.logger.warning("No 'reads_list' column in input_txt. We'll proceed but might get empty eSeq.")
            df["reads_list"] = ""

        # If no eSeq column, create it
        if "eSeq" not in df.columns:
            df["eSeq"] = ""

        # Convert to records list for multi-processing
        ...
        df_out.to_csv(self.output_consensus, index=False)
...
def main():
    ...
    Usage example:
        python astrologer.py \
            ...
            --no-reads-list \
            --min-score 0.0 \
            --output-filter output_filter.csv \
            --input-txt output_filter.csv \
            --ref ref.fa \
            --reads-fa reads.fa \
            ...
            --minimap2-threads 4
    """
    parser = argparse.ArgumentParser(description="Astrologer pipeline (modified to use pandas, output CSV).")
    ...
    parser.add_argument("--z-threshold", type=float, default=1.0, help="Z-threshold for robust outlier detection.")
    parser.add_argument("--min-length", type=int, default=1, help="Minimum length for region.")
    ...
    parser.add_argument("--min-score", type=float, default=10.0, help="Minimum score threshold.")
    parser.add_argument("--output-filter", default="output_filter.csv", help="robust_highcov_filter output (CSV).")

    # -- generate_local_consensus parameters
    parser.add_argument("--input-txt", default="output_filter.csv", help="Input CSV with regions info.")
