...
class MergeUecc:
    ...
    def merge_fasta_files(self, file1, file2, output_file):
        ...
        if os.path.exists(file1) and os.path.getsize(file1) == 0:
            # write file2 to output
            shutil.copy(file2, output_file)
            return
        elif os.path.exists(file2) and os.path.getsize(file2) == 0:
            # write file1 to output
            shutil.copy(file1, output_file)
            return
        elif not os.path.exists(file1) and not os.path.exists(file2):
            logging.error(f"Both input files do not exist: {file1} and {file2}")
            return
        elif os.path.getsize(file1) == 0 and os.path.getsize(file2) == 0:
            # Both input files are empty, return empty fasta file
            print(f"Both input files are empty: {file1} and {file2}")
