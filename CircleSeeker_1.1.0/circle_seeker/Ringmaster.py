...
class Ringmaster:
    def __init__(self, blast_results_file, circular_seq_fasta, Uecc_output_csv, Mecc_output_csv, Cecc_output_csv, uecc_fa, mecc_fa, cecc_fa, num_threads=1):
        # Configure logging
        logging.basicConfig(
            level=logging.INFO,
    ...
    def process_sequences(self, circular_seq_fasta, Uecc_ids, mecc_ids, Cecc_ids, mecc_df, 
                          Cecc_df, Uecc_df, uecc_fa='UeccDNA.fa', mecc_fa='MeccDNA.fa', 
                          cecc_fa='CeccDNA.fa'):
    ...
    def run(self):
        ...
        self.process_sequences(self.circular_seq_fasta, Uecc_ids, mecc_ids, Cecc_ids, mecc_results, Cecc_results, 
                               Uecc_results, self.uecc_fa, self.mecc_fa, self.cecc_fa)

        execution_time = time.time() - start_time
        self.logger.info(f"Analysis completed in {execution_time:.2f} seconds")

        print(f"Processing complete. Output files: {self.Uecc_output_csv}, {self.Mecc_output_csv}, {self.Cecc_output_csv}, {self.uecc_fa}, {self.mecc_fa}, {self.cecc_fa}")
...
if __name__ == "__main__":
    ...
    ringmaster = Ringmaster(args.input, args.input_fasta, args.Uecc_output_csv, args.Mecc_output_csv,
                            args.Cecc_output_csv, args.uecc_fa, args.mecc_fa, args.cecc_fa, 
                            args.threads)
    ringmaster.run()
